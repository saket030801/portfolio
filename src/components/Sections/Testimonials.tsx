...
import {SectionId, quote} from '../../data/data';
import {Quotes} from '../../data/dataDef';
          ...
          <div className="flex flex-col items-center gap-y-6 rounded-xl bg-gray-800/60 p-6 shadow-lg">
            <div
              ...
              {quotes.map((quote, index) => {
                ...
                return <Quote isActive={isActive} key={`${quote.name}-${index}`} quote={quote} />;
              })}
            </div>
...
const Quote: FC<{quote: Quotes; isActive: boolean}> = memo(({quote: {text, name, image}, isActive}) => (
  <div
    className={classNames(
